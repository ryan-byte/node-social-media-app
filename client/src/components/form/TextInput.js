/**
 * JSX input field of the type text
 * 
...
export default function TextInput({label,attributs,value,setValue}){
    return (
        <div className="mb-3">
            <label htmlFor={attributs} className="form-label">{label}</label>
            <input value={value} onChange={(ev)=>setValue(ev.target.value)} required type="text" className="form-control" name={attributs} id={attributs}/>
        </div>
    )
}